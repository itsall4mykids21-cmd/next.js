...
This avoids hydration mismatches without needing `suppressHydrationWarning`.
...
Alternatively, when you can't defer updating via state/effect, you can suppress the mismatch on just the node that renders the pathname. This does not change behavior; it only silences the hydration warning. Prefer isolating updates via state/effect when possible.
...
export default function PathnameInline() {
  const pathname = usePathname()
  return (
    <p>
      Current pathname: <span suppressHydrationWarning>{pathname}</span>
    </p>
  )
}
```
