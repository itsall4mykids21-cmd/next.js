...
pub async fn get_babel_loader_rules(
    ...
    // The working directory that all paths in the programmatic options (e.g. plugin paths) will be
    // resolved relative to.
    loader_options.insert(
        "cwd".to_owned(),
        to_sys_path_str(project_path.clone()).await?.into(),
    );
            ...
            resolve_babel_plugin_react_compiler(next_config, project_path).await?
    {
        ...
        let react_compiler_plugins = serde_json::Value::Array(vec![
